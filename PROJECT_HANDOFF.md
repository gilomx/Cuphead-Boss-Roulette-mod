...
### Interaction prototype (2026-08-21)
...
consumer and the first executor. The local server may accept and queue commands
on its background thread, but it must never instantiate or inspect Unity objects
there. Keep future catalog items out of `Plugin.cs` and add isolated executors
behind the controller instead.

The first two test items use Cuphead's original runtime mapping:
...
camera-relative native entry position. The mod adds only the donor label. The
...
`NativeZeppelinSpawnPattern` selects one of the current difficulty's native
`spawnString` patterns and a lane contained in that pattern. It samples the
...
The donor name is a child world-space `TextMeshPro` using Cuphead's Memphis
font, 86 units above the actor so it follows every movement; unlike the removed
`OnGUI` label, it goes through the gameplay camera filters. The type-A
pink-projectile cadence counter remains
native. Do not replace the lane selection with a fixed test position.

The user likes the current filtered text direction but plans to continue its
visual tuning with another agent. Preserve the child/world-space anchoring as
the working baseline: reverting to `OnGUI` makes the label ignore Cuphead's
camera filters, while detaching the text from the enemy leaves it at a global
screen position.

`CreatorToolsInteractionQueue` is the authoritative runtime queue. Test calls
accept `item`, `donor`, and `quantity`; mixed batches append in arrival order.
For now exactly one actor is active, batches are capped at 50 and total active
plus pending entries at 200. An active row remains in the API until its native
actor is destroyed, then the next pending row is dispatched. Keep these values
isolated here so future settings can add maximum-on-screen, maximum-executing,
and exclusive-item policies without moving business rules into React.

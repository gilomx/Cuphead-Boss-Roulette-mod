...
until its native actor is destroyed, then the next pending row may dispatch.
The random test uses varying 1.25–3.25 second intervals without building a
backlog. Spawning is blocked during loading, real pause, defeat/end-of-level and
the first three seconds after `_OnLevelStart`; actors already visible remain
frozen on defeat and are cleaned when the `Level` is destroyed.

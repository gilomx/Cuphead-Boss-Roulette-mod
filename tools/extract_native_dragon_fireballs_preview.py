...
PYDEPS = ROOT / "obj" / "interaction-pydeps-py312"
if not PYDEPS.is_dir():
    PYDEPS = ROOT / "obj" / "interaction-pydeps"
...
DEFAULT_SPRITE = "dragon_meteor_forward_0007"
DEFAULT_OUTPUT = (
    ROOT / "assets" / "creator-tools" / "interactions"
    / "dragon-fireballs.png"
)
COMMON_BUNDLES = (
    Path(
        r"C:\Program Files (x86)\Steam\steamapps\common\Cuphead\Cuphead_Data"
        r"\StreamingAssets\AssetBundles\atlas_dragonlevel_nobg"
    ),
    Path(
...
def default_bundle():
    ...
    raise FileNotFoundError(
        "Could not find atlas_dragonlevel_nobg; pass its location with --bundle."
    )


def parse_args():
    parser = argparse.ArgumentParser(
        description="Extract the native Dragon fireball preview."
    )

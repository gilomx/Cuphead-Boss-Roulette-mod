...
DEFAULT_SPRITE = "robot_ph1_bombot_0001"
DEFAULT_OUTPUT = (
    ROOT / "assets" / "creator-tools" / "interactions"
    / "robot-homing-bomb.png"
)
COMMON_BUNDLES = (
    Path(
        r"C:\Program Files (x86)\Steam\steamapps\common\Cuphead\Cuphead_Data"
        r"\StreamingAssets\AssetBundles\atlas_robotlevel_hq"
    ),
    Path(
...
def default_bundle():
    ...
    raise FileNotFoundError(
        "Could not find atlas_robotlevel_hq; pass its location with --bundle."
    )


def parse_args():
    parser = argparse.ArgumentParser(
        description="Extract the native Dr. Kahl homing bomb preview."
    )
    parser.add_argument("--bundle", type=Path, help="Path to atlas_robotlevel_hq.")

...
DEFAULT_SPRITE = "tallfrog_firefly_idle_0001"
DEFAULT_OUTPUT = (
...
COMMON_BUNDLES = (
    Path(
        ...
        r"\StreamingAssets\AssetBundles\atlas_frogslevel"
    ),
    Path(
...
def default_bundle():
    ...
    raise FileNotFoundError(
        "Could not find atlas_frogslevel; pass its location with --bundle."
    )


def parse_args():
    parser = argparse.ArgumentParser(
        description="Extract the native Ribby and Croaks firefly preview."
    )
    parser.add_argument("--bundle", type=Path, help="Path to atlas_frogslevel.")

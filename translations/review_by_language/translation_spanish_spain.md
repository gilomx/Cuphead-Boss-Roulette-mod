# Plantilla de traducción — Español de España (`SpanishSpain`)
...
| `challenge.no_dash` | DASH | `TutorialDash` | `ACELERÓN` | `IN-GAME` |
| `challenge.no_mini_plane` | SHRINK | `TutorialShrink` | `ENCOGE` | `IN-GAME` |
| `challenge.mini_plane_only` | SHRINK | `TutorialShrink` | `ENCOGE` | `IN-GAME` |
| `challenge.no_ex` | EX MOVE | `TutorialExShot` | `ATAQUE EX` | `IN-GAME` |
| `challenge.no_peashooter` | PEASHOOTER terrestre | `level_weapon_peashot_name` | `GUISANTEADORA` | `IN-GAME` |
...
| `challenge.no_bombs` | Minibombas en frase visible | `Canteen_Tooltip_ShmupWeapons` | `¦Ahora vuestros aviones están equipados con minibombas! ¦Cambiad de arma en cualquier momento del combate!` | `IN-GAME` |
| `challenge.no_bombs` | Nombre interno de la bomba del avión | `plane_weapon_bomb_name` | `BOMB` | `Do Not Translate` |
| `challenge.black_and_white` | BLACK & WHITE | `OptionMenuFilterBlackWhite` | `BLANCO/NEGRO` | `IN-GAME` |

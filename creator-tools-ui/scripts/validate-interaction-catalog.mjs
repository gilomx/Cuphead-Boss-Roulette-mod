...
for (const match of groupsSource.matchAll(/((?:\s*case "[^"]+":)+)\s*return (Light|Strong|MiniBoss|Challenge|Help);/g)) {
  const group = { Light: "light", Strong: "strong", MiniBoss: "mini_boss", Challenge: "challenge", Help: "help" }[match[2]];
...
for (const match of view.matchAll(/\{\s*id:\s*"([^"]+)"([\s\S]*?)\}/g)) {
  ...
  if (category !== "help" && category !== "attack" && category !== "mini_boss" && category !== "challenge") {
  ...
  if (!image || !/^\/assets\/creator-tools\/(?:charms|interactions|modifiers|supers)\/[a-z0-9_-]+\.png$/.test(image)) {

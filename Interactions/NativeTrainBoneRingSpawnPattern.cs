...
{
    internal sealed class NativeTrainBoneRingSpawnParameters
    {
        internal Vector2 Position;
        internal float UpSpeed;
        internal float HorizontalSpeed;
        internal float Gravity;
    }

    internal static class NativeTrainBoneRingSpawnPattern
    {
        ...
        internal static bool TryCreate(
            ...
            out NativeTrainBoneRingSpawnParameters parameters,
            out string error)
        {
            parameters = null;
            error = null;
            try
            {
                ...
                var properties = LevelProperties.Train.GetMode(mode);
                if (properties == null || properties.CurrentState == null ||
                    properties.CurrentState.engine == null)
                    throw new InvalidOperationException(
                        "Cuphead's native Phantom Express engine properties are unavailable.");

                var engine = properties.CurrentState.engine;
                var cameraScale = Mathf.Max(0.01f, camera.orthographicSize / 360f);
                parameters = new NativeTrainBoneRingSpawnParameters
                {
                    Position = ChoosePosition(camera, occupiedSpawnPositions),
                    UpSpeed = engine.projectileUpSpeed * cameraScale,
                    HorizontalSpeed = engine.projectileXSpeed * cameraScale,
                    Gravity = engine.projectileGravity * cameraScale
                };
                return true;
            }

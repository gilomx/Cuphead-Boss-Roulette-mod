...
{
    internal sealed class BeppiBalloonDogInteractionExecutor :
        ICreatorToolsInteractionExecutor
    {
        private readonly NativeBeppiBalloonDogCache nativeCache;
        private readonly Func<bool> canSpawn;
        private readonly Action<string> logWarning;
        private readonly List<NativeBeppiBalloonDogSpawn> activeSpawns =
            new List<NativeBeppiBalloonDogSpawn>();

        internal BeppiBalloonDogInteractionExecutor(
            ...
            Action<string> logWarning)
        {
            ...
            nativeCache = new NativeBeppiBalloonDogCache(
                coroutineHost,
                canPreload,
                canSpawn,
                logInfo,
        ...
        public bool Supports(string item)
        {
            return string.Equals(
                item,
                CreatorToolsInteractionIds.BeppiBalloonDog,
                StringComparison.Ordinal) || string.Equals(item,
                CreatorToolsInteractionIds.BeppiPinkBalloonDog, StringComparison.Ordinal);
        }
            ...
            out string error)
        {
            ...
            if (!Supports(item))
            {
                ...
                error = "The Beppi balloon dog executor does not " +
                    "support " + item + ".";
                return false;
            }
            ...
            if (!NativeBeppiBalloonDogSpawnPattern.TryCreate(
                ReservedPositions(),
                out parameters,
                out error))
                return false;

            NativeBeppiBalloonDogSpawn spawned;
            if (!nativeCache.TrySpawn(
                item == CreatorToolsInteractionIds.BeppiPinkBalloonDog || UnityEngine.Random.value < 0.2f,
                parameters,
                donor,
                out spawned,
                out error))

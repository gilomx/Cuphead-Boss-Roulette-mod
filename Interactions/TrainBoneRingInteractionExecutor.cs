...
{
    internal sealed class TrainBoneRingInteractionExecutor :
        ICreatorToolsInteractionExecutor
    {
        private readonly NativeTrainBoneRingCache nativeCache;
        private readonly Func<bool> canSpawn;
        private readonly Action<string> logWarning;
        private readonly List<NativeTrainBoneRingSpawn> activeSpawns =
            new List<NativeTrainBoneRingSpawn>();

        internal TrainBoneRingInteractionExecutor(
            ...
            Action<string> logWarning)
        {
            ...
            nativeCache = new NativeTrainBoneRingCache(
                coroutineHost,
                canPreload,
                canSpawn,
                logInfo,
        ...
        public bool Supports(string item)
        {
            return string.Equals(
                item,
                CreatorToolsInteractionIds.TrainBoneRing,
            ...
            out string error)
        {
            ...
            if (!Supports(item))
            {
                ...
                error = "The Phantom Express bone ring executor does not " +
                    "support " + item + ".";
                return false;
            }
            ...
            if (!NativeTrainBoneRingSpawnPattern.TryCreate(
                ReservedPositions(),
                out parameters,
                out error))
                return false;

            NativeTrainBoneRingSpawn spawned;

...
The preferred local port is 18081, but if it is already in use, the mod automatically selects another one. Always use COPY URL instead of typing the address manually. The overlay runs through a local server and does not require an internet connection. Size, order, alignment, opacity and logo settings update live.
...
Always keep the port shown in the URL generated by the mod, since it may be different if the default port is already in use.
...
El puerto local preferido es 18081, pero si ya está ocupado, el mod selecciona otro automáticamente. Usa siempre COPIAR URL en vez de escribir la dirección manualmente. El overlay funciona mediante un servidor local y no requiere conexión a internet. Los ajustes de tamaño, orden, alineación, opacidad y logo se actualizan en vivo.
...
Conserva siempre el puerto que aparece en la URL generada por el mod, ya que puede ser diferente si el puerto predeterminado está ocupado.
